...
class Solver_8_queens:
    ...
    __mut_prob = 1.0
    __board_size = 8
    __code_queen_size = int(np.log2(__board_size))
    __addition_size = 8 - __code_queen_size
    __full_code_queen_size = int(__code_queen_size * __board_size)
    ...
    def __generate_population(self):
        initial_population = np.zeros((self.__pop_size, self.__full_code_queen_size), dtype=np.ubyte)
    ...
    def __count_fit_values(self, population):
        ...
        for i, chromosome in enumerate(population):
            fit_values[i] = (self.__fit_function(self.__get_chromosome_int_code(chromosome)))
    ...
    def __reproduction(self, population, fit_values):
        ...
        parents = np.zeros((len(population), self.__full_code_queen_size), dtype=np.ubyte)
    ...
    def __crossover(self, parents):
        children = np.zeros((len(parents), self.__full_code_queen_size), dtype=np.ubyte)
        ...
        for x in range(0, int(self.__pop_size / 2)):
            if random.randrange(0, 100) < self.__cross_prob * 100:
                ...
                point = random.randrange(1, self.__full_code_queen_size)
    ...
    def __generate_visualization(self, solution):
        result = ''
        for gen in self.__get_chromosome_int_code(solution):
            result += '+' * gen + 'Q' + '+' * (self.__board_size - gen - 1) + '\n'

        return result

    def __fit_function(self, chromosome_int_code):
        ...
        for x1, y1 in enumerate(chromosome_int_code):
            for x2 in range(x1 + 1, len(chromosome_int_code)):
                if self.is_queen_conflict(x1, y1, x2, chromosome_int_code[x2]):
                    count += 1

        return 1 / (count + 1)

    @staticmethod
    def is_queen_conflict(x1, y1, x2, y2):
        if x1 == x2 or y1 == y2:
            return False
    ...
    def __get_chromosome_int_code(self, chromosome_bin_code):
        full_bin_code = self.__generate_full_code(chromosome_bin_code[:self.__code_queen_size])

        for k in range(self.__code_queen_size, len(chromosome_bin_code), self.__code_queen_size):
            full_bin_code = np.concatenate((
                full_bin_code,
                self.__generate_full_code(chromosome_bin_code[k:][:self.__code_queen_size])
            ))
